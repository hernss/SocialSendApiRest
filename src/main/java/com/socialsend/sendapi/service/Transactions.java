...
public class Transactions {
	...
	public Response<Double> getvalue(@PathParam("txid") String txid) {
				...
				if(sp.getType().equals("nonstandard")) {
					...
					for(Vout v : tx1.getVout()) {
						if(v.getScriptPubKey().getType().equals("pubkey")) {
							...
							if(!v.getScriptPubKey().getAddresses().get(0).equals("")) {
								addressDestino = v.getScriptPubKey().getAddresses().get(0);
							}
						}
					...
					if(!tx1.getVin().isEmpty()) {
						...
						if((!tx2.getVout().isEmpty()) && tx2.getVout().get((int)vout).getScriptPubKey().getAddresses().get(0).equals(addressDestino)) {
					...
					double value = tx1.getVout().get(0).getValue();
					String addrDestino = sp.getAddresses().get(0);
					r = new Response<Double>(value);
					r.setMessage("AddrTo:" + addrDestino);
				}
			}else {
				r = new Response<Double>(null);
				r.setStatus("ERROR");
